...
// -------- ROUTINE (AJAX) --------
let routineData = {};
let routineDaysOrder = []; // preserve day order
...
function fetchRoutine() {
  fetch("routine.php?action=fetch")
    ...
    .then((data) => {
      ...
      routineDaysOrder = [];
      data.forEach((r) => {
        if (!routineData[r.day]) {
          routineData[r.day] = [];
          routineDaysOrder.push(r.day); // preserve order of first appearance
        }
...
function addRoutine() {
  ...
  fetch("routine.php", {
    ...
    .then((res) => {
      alert(res);
      fetchRoutine(); // নতুন ক্লাস দিন অনুযায়ী নিচে দেখাবে
    });
...
function downloadPDF() {
  ...
  html2pdf()
    .set({ margin: 0.5, filename: "Routine.pdf", html2canvas: { scale: 2 } })
    .from(clone)
    .save();
}
